...
void FastHessian::getIpoints()
{
	...
	for (int o = 0; o < octaves; ++o) for (int i = 0; i <= 1; ++i)
	{
		...
		for (int r = 0; r < t->height; ++r)
		{
			...
			{
				float candid = -1;
				if (isExtremum(r, c, t, m, b, &candid))
				{
					interpolateExtremum(r, c, t, m, b, candid);
				}
			}
		}
	}
...
int FastHessian::isExtremum(int r, int c, ResponseLayer *t, ResponseLayer *m, ResponseLayer *b, float* candid)
{
	// bounds check
	...
	if (candidate < thresh) {// I want to keep the "candidate value and use it to identify the stronger features
		*candid = -1;
		return 0;
	} else
		*candid = candidate;
...
void FastHessian::interpolateExtremum(int r, int c, ResponseLayer *t, ResponseLayer *m, ResponseLayer *b, float candid)
{
	...
	if( fabs( xi ) < 0.5f  &&  fabs( xr ) < 0.5f  &&  fabs( xc ) < 0.5f )
	{
		...
		ipt.hessian = static_cast<float>(candid);
//		ipts.push_back(ipt); // Unsorted push_back
		sortHessian(&ipt); // Sorted push_back
...
void FastHessian::sortHessian(Ipoint* ipt)
{
	for (std::vector<Ipoint>::iterator it = ipts.begin(); it != ipts.end(); ++it) {
		if (ipt->hessian <= it->hessian) {
			ipts.insert(it, *ipt);
			return;
		}
	}
